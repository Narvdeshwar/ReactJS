import {
  ...
  require_tty,
  warnDeprecation
} from "./chunk-U5SQXQXS.js";
import {
  require_node_path
} from "./chunk-AW2EMQN5.js";
import {
  require_path
} from "./chunk-PSMGTXPV.js";
import {
  __toESM
...
var ObjectEntity = class extends ExpressionEntity {
  ...
  deoptimizePath(path) {
    ...
    (_a = this.prototypeExpression) == null ? void 0 : _a.deoptimizePath(path.length === 1 ? [...path, UnknownKey] : path);
  }
...
var LocalVariable = class extends Variable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker) {
    ...
    recursionTracker.withTrackedEntityAtPath(path, this.init, () => this.init.deoptimizeArgumentsOnInteractionAtPath(interaction, path, recursionTracker), void 0);
  }
  deoptimizePath(path) {
  ...
  hasEffectsOnInteractionAtPath(path, interaction, context) {
    switch (interaction.type) {
      case INTERACTION_ACCESSED: {
        if (this.isReassigned)
          return true;
        return !context.accessed.trackEntityAtPathAndGetIfTracked(path, this) && this.init.hasEffectsOnInteractionAtPath(path, interaction, context);
      }
      case INTERACTION_ASSIGNED: {
        if (this.included)
          return true;
        ...
        if (this.isReassigned)
          return true;
        return !context.assigned.trackEntityAtPathAndGetIfTracked(path, this) && this.init.hasEffectsOnInteractionAtPath(path, interaction, context);
      }
      case INTERACTION_CALLED: {
        if (this.isReassigned)
          return true;
        return !(interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path, interaction.args, this) && this.init.hasEffectsOnInteractionAtPath(path, interaction, context);
      }
    }
  }
  include() {
...
var ParameterVariable = class extends LocalVariable {
  ...
  deoptimizeArgumentsOnInteractionAtPath(interaction, path) {
    if (path.length >= 2 || this.entitiesToBeDeoptimized.has(UNKNOWN_EXPRESSION) || this.deoptimizationInteractions.length >= MAX_TRACKED_INTERACTIONS || path.length === 1 && (this.deoptimizedFields.has(UnknownKey) || interaction.type === INTERACTION_CALLED && this.deoptimizedFields.has(path[0]))) {
      deoptimizeInteraction(interaction);
      return;
    }
...
function requireConstants() {
  ...
  const path = import_path.default;
  const WIN_SLASH = "\\\\/";
...
function requirePicomatch$1() {
  ...
  picomatch2.matchBase = (input, glob, options, posix2 = utils2.isWindows(options)) => {
  ...
  function classBody(node, position, buffer) {
    ...
    if (bodyPosition) {
      ...
      for (let index = 0; index < length; index++) {
        ...
        body.push(convertNode2(node, (buffer[nodePosition + 3] & 1) === 0 ? scope.instanceScope : scope, nodePosition, buffer));
      }
    }
  },
...
function convertNodeList(parent, parentScope, position, buffer) {
  ...
  for (let index = 0; index < length; index++) {
    ...
    list.push(nodePosition ? convertNode2(parent, parentScope, nodePosition, buffer) : null);
  }
  return list;
}
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-EF3CKYFG.js");
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-7O5LU5OL.js.map
